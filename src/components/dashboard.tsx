...
import HeathCheckBarChart from './bar-chart';
...
export default function Dashboard() {
  ...
  const createHealthCheckSummary = (
      ...
      const status = value === workingStatusId ? 'Working' : "Failing";
      return { label, value, status, count };
    }
  ...
  const fetchData = React.useCallback(async () => {
    ...
    response.data.forEach((item, _) => {
        ...
        const summary = createHealthCheckSummary(item.label, item.value, item.count)
    ...
    <Fragment>
        ...
        <Container>
         ...
         <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3}}>
          ...
          <br />
          <Item>
          <HeathCheckBarChart summaries={summaries}/>
          </Item>
         <br />
         <br />
